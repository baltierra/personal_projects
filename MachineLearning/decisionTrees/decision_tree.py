...
def go(training_filename, testing_filename):
    ...
    for index, row in df_test.iterrows():
        label = apply_tree(index,row, tree)
        rv.append(label)

    return rv


def apply_tree(index,row, tree):
    '''
    ...
    #Recursion
        ...
        if index == 11:
            print("\nSplitting column:",tree.split_col)
            print("\nValue at split col:",row[tree.split_col] )
            print("Test value at index 11", test_val)
        if test_val in tree.children:
            for child_name, child_node in tree.children.items():
                if test_val == child_name:
                    if index == 11:
                        print("\nChild at index 11:", child_name)
                        print("Test value at index 11:", test_val)
                        print("Label",child_node.class_label )
                    return apply_tree(index, row, child_node)
        else:
            return tree.class_label
