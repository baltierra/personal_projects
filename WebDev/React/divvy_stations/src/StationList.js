...
class StationList extends React.Component {
  ...
  handleStationClick = (e) => {
    ...
    const selectedStation = this.props.stations.find(
      (station) => station.station_id === stationIdentifier
    );
    if (selectedStation) {
      //#3: Update this component's state so that this.state.selectedStationName
      //will contain the selected station's name.
      this.setState({ selectedStationName: selectedStation.name });
  ...
  getStationDetails = async (stationIdentifier) => {
    ...
    const details = results.data.stations.find(
      (station) => station.station_id === stationIdentifier
    );
    //#4: Update this component's state so that this.state.selectedStationDetails
    //will have the same value as the 'details' variable above
    ...
    return (
      <div className="row mt-5 justify-content-center">
        <div className="col-sm-5">
          <ul className="list-group">{stationMarkup}</ul>
        </div>

        <div className="col-sm-4">
          <StationDetails
            details={this.state.selectedStationDetails}
            name={this.state.selectedStationName}
          />
        </div>
      </div>
    );
